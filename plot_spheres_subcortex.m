...
[contact_colours, colours_rgb] = map_values_to_cmap([df_query.per; 0.5; 0.9], 'viridis');
...
df_query = df((strcmp(df.loc, 'STN') | strcmp(df.loc, 'GP')) & strcmp(df.label, 'pkg_tremor') & strcmp(df.classification, 'True'), :);
[contact_colours, colours_rgb] = map_values_to_cmap([df_query.per; 0.5; 0.75], 'viridis');
contact_colours = contact_colours(1:end-2, :);
radius = 0.3;
[x, y, z] = sphere(100);
for row=1:height(df_query)
...
df_query = df((strcmp(df.loc, 'STN') | strcmp(df.loc, 'GP')) & strcmp(df.label, 'pkg_bk') & strcmp(df.classification, 'True'), :);
