    ...
    for symptom in ["pkg_bk", "pkg_dk", "pkg_tremor"]:
        ...
        for range_low in range(5, 34):
            for range_high in range(6, 36):
                ...
                ind_band = df_all[df_all["sub"] == sub][[f"ch_subcortex_welch_psd_{int(i)}_mean" for i in range(range_low, range_high)]].apply(lambda x: 10**x).sum(axis=1).values
...
for idx_sym, symptom in enumerate(["pkg_bk", "pkg_dk", "pkg_tremor"]):
    for idx_plt, plt_val in enumerate(["per_ind_band", "p_val"]):
        ...
        df_plt = df_per.query(f"symptom == @symptom")[["rang_low", "range_high", "per_ind_band", "p_val", "sub"]]
        # remove nans
        df_plt = df_plt.dropna()

        df_plt = df_plt.groupby(["rang_low", "range_high"])[["per_ind_band", "p_val"]].mean().reset_index()
        
        color_ = df_plt[plt_val].values
        if plt_val == "p_val":
            color_ = np.log10(color_)
        plt.scatter(df_plt["rang_low"], df_plt["range_high"], c=color_, cmap="jet", s=50, marker="s")
        plt.xlabel("Range low [Hz]")
        plt.ylabel("Range high [Hz]")
        if plt_val == "per_ind_band":
            min_low, min_high = df_plt.loc[df_plt[plt_val].idxmax(), ["rang_low", "range_high"]]
            plt_title = "Corr. coef."
            plt.title(f"{plt_title}\n{symptom} max: {int(min_low)}-{int(min_high)}\n corr.: {df_plt[plt_val].max():.2f}")
        else:
            df_plt_ = df_plt.query("per_ind_band > 0")
            min_low, min_high = df_plt_.loc[df_plt_[plt_val].idxmin(), ["rang_low", "range_high"]]
            plt_title = "p-value"
