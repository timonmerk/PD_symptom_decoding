...
# get Stats
def print_stats(CLASS_, LOC_1, LOC_2):
    for label_ in df["label"].unique():
        _, p = nm_stats.permutationTest(
            df.query(f"classification == {CLASS_}").query(f"label == @label_").query(f"loc == '{LOC_1}'")["per"].values,
            df.query(f"classification == {CLASS_}").query(f"label == @label_").query(f"loc == '{LOC_2}'")["per"].values,
            False, None, 5000
        )
        print(f"{label_} p: {p}")
        print("mean STN", df.query(f"classification == {CLASS_}").query(f"label == @label_").query(f"loc == '{LOC_1}'")["per"].mean().round(2))
        print("std STN", df.query(f"classification == {CLASS_}").query(f"label == @label_").query(f"loc == '{LOC_1}'")["per"].std().round(2))
        print("mean GP", df.query(f"classification == {CLASS_}").query(f"label == @label_").query(f"loc == '{LOC_2}'")["per"].mean().round(2))
        print("std GP", df.query(f"classification == {CLASS_}").query(f"label == @label_").query(f"loc == '{LOC_2}'")["per"].std().round(2))
        print()

#print_stats(True, "STN", "GP")
#print_stats(False, "STN", "GP")
#print_stats(True, "SC", "MC")
print_stats(False, "SC", "MC")
...
print_surfice(
    ...
    list(
        np.clip(np.abs(df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_bk'")["per"].values.round(2))
    , 0.1, 0.9)),
    [0.5 for _ in range(len(list(
        np.abs(df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_bk'")["per"].values.round(2))
    )))]
)

print_surfice(
     list(
        np.abs(df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_dk'")["x"].values.round(2))
    ),
    list(
        df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_dk'")["y"].values.round(2)
    ),
    list(
        df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_dk'")["z"].values.round(2)
    ),
    list(
        np.clip(np.abs(df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_dk'")["per"].values.round(2))
    , 0.0, 0.9)),
    [0.5 for _ in range(len(list(
        np.abs(df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_dk'")["per"].values.round(2))
    )))]
)
...
print_surfice(
     list(
        np.abs(df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_tremor'")["x"].values.round(2))
    ),
    list(
        df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_tremor'")["y"].values.round(2)
    ),
    list(
        df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_tremor'")["z"].values.round(2)
    ),
    list(
        np.clip(np.abs(df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_tremor'")["per"].values.round(2))
    , 0.0, 0.8)),
    [0.5 for _ in range(len(list(
        np.abs(df.query ("loc == 'MC' or loc == 'SC'").query("classification == False").query("label == 'pkg_tremor'")["per"].values.round(2))
    )))]
)
