...
sb.regplot(data=data_plt, x=col_pkg, y=col_plt, color="#846AA1")
...
plt.subplot(1, 3, 2)
col_plt = "updrs_tremor"
col_pkg = "pkg_tremor_mean"
...
sb.regplot(data=data_plt, x=col_pkg, y=col_plt, scatter_kws={'s':14*1.7}, color="#29AF7F")
#rho, p = stats.spearmanr(data_plt[col_pkg], data_plt[col_plt])
_, p = nm_stats.permutationTestSpearmansRho(
    data_plt[col_pkg], data_plt[col_plt], False, None, 5000, Spearman=False
)
...
#)
plt.title(f"Tremor\nrho={rho:.2f}, p={p:.2f}")
# turn off upper and right lines
plt.gca().spines['top'].set_visible(False)
plt.gca().spines['right'].set_visible(False)
...
idx_not_inf = np.isfinite(df_out[col_pkg])
data_plt = df_out[idx_not_none].groupby(["sub"])[[col_plt, col_pkg]].mean().reset_index()
sb.regplot(data=data_plt[idx_not_inf], x=col_pkg, y=col_plt, color="#BDDF26")
...
plt.tight_layout()
plt.savefig(os.path.join(PATH_FIGURES, "pkg_corr_fig_1.pdf"))  
plt.show(block=True)
