...
probas = probas[:, :]
# smooth the probas with a running average of 8
probas_smoothed = np.array([np.convolve(probas[:, i], np.ones(8)/8, mode='same') for i in range(probas.shape[1])]).T[::8, :]
...
def update(frame):
    ...
    for i, line in enumerate(lines):
        line.set_data(range(end - start), probas_smoothed[start:end, i])
...
# Create animation
n_frames = len(probas_smoothed)  # Total number of frames
...
ani.save("sliding_window_animation_smoothed.mp4", writer="ffmpeg", fps=5)
