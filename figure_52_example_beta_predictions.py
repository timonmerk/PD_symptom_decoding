...
for label in ["pkg_dk", "pkg_bk", "pkg_tremor"]:
    ...
    for sub in np.sort(list(d_out.keys())):
        ...
        ind_band = df_sub[[f"ch_subcortex_welch_psd_{int(i)}_mean" for i in range(int(ind_peaks_short[sub]-2.5), int(ind_peaks_short[sub]+2.5))]].mean(axis=1).values
...
df_comp = df_comp.melt(id_vars=["sub", "label"], value_vars=["corr_ind", "corr_pr"], var_name="type", value_name="value")
